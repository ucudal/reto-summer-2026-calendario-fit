(function () {
    function useGroupManagement({
        ...
        function findCalendarForYear(
            ...
            const academicSemester = String(selectedSemester || "");
            const year = String(selectedYear || "");
            const lectiveSemester = String(selectedLectiveSemester || "");
            const semesterLabel = lectiveSemester === "2" ? "2do semestre" : "1er semestre";
            const expectedLectiveTerm = selectedLectiveYear
                ? `${semesterLabel} ${String(selectedLectiveYear)}`
                : "";

            const byYear = calendars.filter((c) => {
                const id = String(c.id || "");
                const matchesAcademic =
                    (academicSemester === "1" || academicSemester === "2")
                        ? id.startsWith(`s${academicSemester}y${year}`)
                        : id.includes(`y${year}`);

                if (!matchesAcademic) return false;
                if (!expectedLectiveTerm) return true;

                return String(c.lectiveTerm || "").trim() === expectedLectiveTerm;
            });
        ...
        function replaceSubjectGroupsInCalendar(
            ...
            const target = findCalendarForYear(
                selectedYear,
                prevData.calendars,
                selectedSemester,
                selectedLectiveSemester,
